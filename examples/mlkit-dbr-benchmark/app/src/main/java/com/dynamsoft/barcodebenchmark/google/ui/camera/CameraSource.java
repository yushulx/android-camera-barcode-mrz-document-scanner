 ...
 * {@link FrameProcessor}. This receives preview frames
 * from the camera at
 * a specified rate, sending those frames to the detector as fast as it is able
 * to process those
 * frames.
 * <p/>
 * This camera source makes a best effort to manage processing on preview frames
 * as fast as
 * possible, while at the same time minimizing lag. As such, frames may be
 * dropped if the detector
 * is unable to keep up with the rate of frames generated by the camera. You
 * should use
 * {@link CameraSource.Builder#setRequestedFps(float)} to specify a frame rate
 * that works well with
 * the capabilities of the camera hardware and the detector options that you
 * have selected. If CPU
 * utilization is higher than you'd like, then you may want to consider reducing
 * FPS. If the camera
 * preview or detector results are too "jerky", then you may want to consider
 * increasing FPS.
 * <p/>
...
public class CameraSource {
     ...
     * size aspect
     * ratio is less than this tolerance, they are considered to be the same aspect
     * ratio.
     */
    ...
    /**
     * Rotation of the device, and thus the associated preview images captured from
     * the device.
    ...
    // These values may be requested by the caller. Due to hardware limitations, we
    // may need to
    ...
    // these aren't used outside of the method that creates them, they still must
    // have hard
    ...
    /**
     * Dedicated thread and associated runnable for calling into the detector with
     * frames, as the
     ...
     * associated byte
     * buffer. We use byte buffers internally because this is a more efficient way
     * to call into
    ...
    public static class Builder {
         ...
         * Camera preview
         * images will be streamed to the associated detector upon starting the camera
         * source.
         */
        public Builder(Context context, FrameProcessor processor) {
            if (context == null) {
        ...
        /**
         * Sets the requested frame rate in frames per second. If the exact requested
         * value is not
         ...
         * exact desired
         * values are not available options, the best matching available options are
         * selected.
         * Also, we try to select a preview size which corresponds to the aspect ratio
         * of an
         * associated full picture size, if applicable. Default: 1024x768.
         */
        public Builder setRequestedPreviewSize(int width, int height) {
            // Restrict the requested range to something within the realm of possibility.
            // The
            // choice of 1000000 is a bit arbitrary -- intended to be well beyond
            // resolutions that
        ...
        public CameraSource build() {
            mCameraSource.mFrameProcessor = mCameraSource.new FrameProcessingRunnable(mProcessor);
    ...
    public interface ShutterCallback {
         ...
         * sensor. This
         * is a good opportunity to play a shutter sound or give other feedback of
         * camera operation.
         * This may be some time after the photo was triggered, but some time before the
         * actual data
         * is available.
         */
        void onShutter();
    ...
    public interface PictureCallback {
        /**
         * Called when image data is available after a picture is taken. The format of
         * the data
         * is a jpeg binary.
         */
     ...
     *
     * @throws IOException if the camera's preview texture or display could not be
     *                     initialized
     */
     ...
     * surface holder is used for the preview so frames can be displayed to the
     * user.
     *
     * @param surfaceHolder the surface holder to use for the preview frames
     * @throws IOException if the supplied surface holder could not be used as the
     *                     preview display
     */
    @RequiresPermission(Manifest.permission.CAMERA)
     ...
     * <p/>
     * Call {@link #release()} instead to completely shut down this camera source
     * and release the
        ...
        synchronized (mCameraLock) {
            ...
            if (mCamera != null) {
                ...
                try {
                    ...
                    // developer doesn't
                    // want to display a preview we use a SurfaceTexture if we are running at least
                    // Honeycomb.
    ...
    /**
     * Initiates taking a picture, which happens asynchronously. The camera source
     * should have been
     * activated previously with {@link #start()} or {@link #start(SurfaceHolder)}.
     * The camera
     * preview is suspended while the picture is being taken, but will resume once
     * picture taking is
     * done.
     *
     ...
     *
     * @return current focus mode. This value is null if the camera is not yet
     *         created. Applications should call {@link
     ...
     * (between {@link #start()} or {@link #start(SurfaceHolder)} and before
     * {@link #stop()} or {@link #release()}).
     * <p/>
     * <p>
     ...
     *
     * @param cb the callback to run
     * @return {@code true} if the operation is supported (i.e. from Jelly Bean),
    ...
    /**
     * Wraps the final callback in the camera sequence, so that we can automatically
     * turn the camera
    ...
    /**
     * Wraps the camera1 auto focus callback so that the deprecated API isn't
     * exposed.
     */
    ...
    /**
     * Wraps the camera1 auto focus move callback so that the deprecated API isn't
     * exposed.
     */
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
     ...
     * the preview
     * size and the picture size of the camera together, because these need to have
     * the same aspect
     * ratio. On some hardware, if you would only set the preview size, you will get
     * a distorted
     * image.
     *
     ...
     * avoid distorted
     * preview images on some devices, the picture size must be set to a size that
     * is the same
     * aspect ratio as the preview size or the preview may end up being distorted.
     * If the picture
     * size is null, then there is no picture size with the same aspect ratio as the
     * preview size.
     */
     ...
     * not a corresponding picture size of the same aspect ratio. If there is a
     * corresponding
     * picture size of the same aspect ratio, the picture size is paired up with the
     * preview size.
     * <p/>
     * This is necessary because even if we don't use still pictures, the still
     * picture size must be
     * set to a size that is the same aspect ratio as the preview size we choose.
     * Otherwise, the
     ...
     * process frames when
     * available from the camera. This is designed to run detection on frames as
     * fast as possible
     * (i.e., without unnecessary context switching or waiting on the next frame).
     * <p/>
     * While detection is running on a frame, new frames may be received from the
     * camera. As these
     * frames come in, the most recent frame is held onto as pending. As soon as
     * detection and its
     * associated processing are done for the previous frame, detection on the
     * mostly recently
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        FrameProcessingRunnable(FrameProcessor processor) {
            mProcessor = processor;
        }
         ...
         * frame buffer
         * (if present) back to the camera, and keeps a pending reference to the frame
         * data for
         * future use.
         */
         ...
         * hasn't been
         * received yet. Once it is available, we transfer the frame info to local
         * variables and
         * run detection on that frame. It immediately loops back for the next frame
         * without
         * pausing.
         * <p/>
         * If detection takes longer than the time in between new frames from the
         * camera, this will
         * mean that this loop will run without ever waiting on a frame, avoiding any
         * context
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    mProcessor.process(data, mPreviewSize.getWidth(), mPreviewSize.getHeight(), mRotation);
                } catch (Throwable t) {
