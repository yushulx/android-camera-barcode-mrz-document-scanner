...
public class CameraSourcePreview extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (width > 0 && height > 0) {
            float widthRatio = (float) layoutWidth / width;
            float heightRatio = (float) layoutHeight / height;

            // To fill the screen, we take the larger ratio
            float scale = Math.max(widthRatio, heightRatio);

            childWidth = (int) (width * scale);
            childHeight = (int) (height * scale);
        }
