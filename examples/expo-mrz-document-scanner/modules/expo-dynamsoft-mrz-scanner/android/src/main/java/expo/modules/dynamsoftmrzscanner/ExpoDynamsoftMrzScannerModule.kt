...
/**
 * In-app Expo module that exposes the Dynamsoft MRZ Scanner **native** Android
 ...
 */
class ExpoDynamsoftMrzScannerModule : Module() {

  companion object {
    private const val TAG = "ExpoMrzScanner"
  }
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("initLicense") { license: String?, promise: Promise ->
      ...
      val ok = IdScannerEngine.getInstance().initLicense(key)
      promise.resolve(
        mapOf(
          "success" to ok,
    ...
    AsyncFunction("scanFile") { uri: String, promise: Promise ->
      ...
      backgroundExecutor.execute {
        try {
          promise.resolve(IdScanResultPayload.fromFileResult(captureUri(uri, context)))
        } catch (e: Exception) {
          Log.e(TAG, "Failed to parse image", e)
          promise.reject("PARSE_FAILED", "Failed to parse image: ${e.message}", e)
        }
      }
    }
  ...
  /** Parse a content URI, a `file://` URI, or a raw absolute path. */
  private fun captureUri(uri: String, context: android.content.Context): com.dynamsoft.cvr.CapturedResult {
    val engine = IdScannerEngine.getInstance()
    return if (uri.startsWith("content://") || uri.startsWith("android.resource://")) {
      engine.parseBitmap(loadBitmap(Uri.parse(uri), context))
    } else {
