...
/** A promise parked by [ExpoDynamsoftMrzScannerModule] while the bridge is open. */
data class Pending(
  val promise: Promise,
  val kind: String,
...
class BridgeResultActivity : Activity() {
  ...
  companion object {
    ...
    const val EXTRA_KIND = "expo.modules.dynamsoftmrzscanner.BRIDGE_KIND"
    const val KIND_SCAN = "scan"
    const val KIND_GALLERY = "gallery"

    /** Single-flight slot shared with [ExpoDynamsoftMrzScannerModule]. */
    @Volatile
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    when (intent.getStringExtra(EXTRA_KIND)) {
      KIND_SCAN -> {
        startActivityForResult(Intent(this, IdScanActivity::class.java), REQUEST)
      }
      KIND_GALLERY -> {
        ...
        startActivityForResult(Intent.createChooser(picker, "Select a document image"), REQUEST)
      }
  ...
  override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
    ...
    if (resultCode != RESULT_OK) {
      val error = data?.getStringExtra(IdScanResultContract.EXTRA_ERROR)
    ...
    when (holder?.kind) {
      KIND_SCAN -> {
        val json = data?.getStringExtra(IdScanResultContract.EXTRA_JSON)
        if (json == null) {
          holder.promise.reject("canceled", "Scan failed or was canceled", null)
        } else {
          try {
            holder.promise.resolve(IdScanResultPayload.fromActivityJson(json))
          } catch (e: Exception) {
        ...
        if (uri == null) {
          ...
          Thread {
            try {
              ...
              val captured = engine.parseBitmap(loadBitmap(uri, holder.applicationContext))
              holder.promise.resolve(IdScanResultPayload.fromFileResult(captured))
            } catch (e: Exception) {
              Log.e(TAG, "Failed to parse picked image", e)
              holder.promise.reject("PARSE_FAILED", "Failed to parse image: ${e.message}", e)
            }
          }.start()
        }
      }
  ...
  private fun loadBitmap(uri: Uri, context: Context): android.graphics.Bitmap {
    val stream: InputStream = context.contentResolver.openInputStream(uri)
      ?: throw IdScannerEngine.ScannerException("Cannot open $uri")
