...
public class GraphicOverlay extends View {
  ...
  // true: the image is displayed with "fit center" semantics (still image preview).
  // false: the image fills and crops the view (camera preview).
  private boolean fitCenterMode = true;
  ...
  /**
   * Selects whether the image is displayed with "fit center" semantics (centered inside the
   * view) or with "fill center" semantics (cropped to fill the view, like a camera preview).
   * Defaults to fit center.
   */
  public void setFitCenterMode(boolean fitCenterMode) {
    synchronized (lock) {
      this.fitCenterMode = fitCenterMode;
      needUpdateTransformation = true;
    }
    postInvalidate();
  }
  ...
  private void updateTransformationIfNeeded() {
    ...
    if (fitCenterMode) {
      // The still-image preview renders the bitmap with "fit center" semantics: the image is
      // scaled to fit inside the view and centered, so the overlay transform must do the same
      // to stay aligned with the displayed bitmap.
      float scale = Math.min((float) getWidth() / imageWidth, (float) getHeight() / imageHeight);
      scaleFactor = scale;
      postScaleWidthOffset = (getWidth() - imageWidth * scale) / 2;
      postScaleHeightOffset = (getHeight() - imageHeight * scale) / 2;
    } else {
      // "Fill center" semantics: the image is scaled to fill the view and the overflow is
      // centered and cropped, matching how the camera preview renders the frames.
      float viewAspectRatio = (float) getWidth() / getHeight();
      float imageAspectRatio = (float) imageWidth / imageHeight;
      if (viewAspectRatio > imageAspectRatio) {
        // The image needs to be vertically cropped to be displayed in this view.
        scaleFactor = (float) getWidth() / imageWidth;
        postScaleHeightOffset = (getHeight() - getWidth() / imageAspectRatio) / 2;
        postScaleWidthOffset = 0;
      } else {
        // The image needs to be horizontally cropped to be displayed in this view.
        scaleFactor = (float) getHeight() / imageHeight;
        postScaleWidthOffset = (getWidth() - getHeight() * imageAspectRatio) / 2;
